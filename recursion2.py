...
#  Description: Program does a variety of recursive functions such as groupSum,
#  groupSum6, groupNoAdj, groupSum5, groupSumClump, splitArray, splitOdd10, split53
...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        return target == 0
    elif start + 1 < len(nums) - 1 and nums[start] == nums[start + 1]:
        index = start
        while nums[index] == nums[start]:
            index += 1
        add = (index - start) * nums[start]
        return groupSumClump(start + (index - start), nums, target-add) or groupSumClump(start + (index - start), nums, target)
    else:
...
def split53(nums):
    ...
    sum1 = 0
    sum2 = 0
    return split53Helper(start, nums, sum1, sum2)
